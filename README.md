...
### v1.01 — 2026-04-01
- Главная: добавлен экран с 5 настраиваемыми виджетами (счётчик заметок по фильтру)
- Виджеты: склонение «штука / штуки / штук» в зависимости от числа
- Настройки: отображение номера версии в нижней части экрана
- Настройки → Настройка почты: ввод SMTP-параметров прямо в интерфейсе (без правки конфигов)
- Хэштеги: цветные метки в карточках заметок (детерминированная палитра)
- При старте приложения открывается экран «Главная»
- Исправлено: планировщик рассылок не запускался под gunicorn — письма теперь отправляются по расписанию
